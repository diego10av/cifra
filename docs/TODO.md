...
>
> Last updated: 2026-05-05 (stint 67.G shipped — full cross-module audit cerrado: Tax-Ops (32 routes) + CRM (17+4 routes) + Settings + Marketing recorridos end-to-end. Bug #16 (tax_entities legal_names con `;` colado por importador stint-34 + whitespace runs) curado en prod + POST endurecido (trim + strip punctuation + collapse whitespace al escribir). Bug #17 (Settings dashboard mostraba "Authentication Missing" rojo desde stint 62 cuando se borró AUTH_PASSWORD env var) arreglado: `/api/system` + `/api/health` + `/settings/page.tsx` + `src/lib/auth.ts` + `src/app/marketing/page.tsx` ahora reflejan el modelo real AUTH_USERS + AUTH_PASS_<USER>. CRM 0 bugs (Rule §14 limpia tras drop crm_companies.entity_id en 67.F). Marketing renders clean. 707/707 tests · 0/517 lint:design · tsc clean. Branch pusheada a origin/main 2afbe4c. Vercel deploy auto-propaga.)

> Earlier: 2026-05-05 (stint 67.F shipped — clear every pending bug + obsolete code. Migration 079 dropped legacy crm_companies.entity_id; cierre del deferred memory item de 2026-04-22.)

> Earlier: 2026-05-04 (stints 67.D + 67.E shipped — VAT module end-to-end audit night. 8 PDFs sintéticos cubriendo cada treatment code, walked lifecycle created → uploading → review → approved → filed → paid → reopened. 5 bugs fixed live: Suspense trap (67.B/B.b/C/C.b/C.c — force-dynamic per-page + drop dead Suspense wrappers en 13 páginas), GROUP BY de aliases en EC sales list, eCDF box 097 always 0 (manual boxes init AFTER formula loop + entity_prorata pre-fill), audit-trail-pdf WinAnsi encoding crash con `→ — “ ” … •`, portal/approve no estampaba approved_at/approved_by. RULE 16 nueva (B2B EU services → OUT_EU_RC determinista). 707/707 tests.)

> Earlier: 2026-05-04 (stint 66 — Rule §14 strict module independence. Removed crm_companies.entity_id (cross-module FK), removed cross-module widgets en /crm "Upcoming this week", grabado en CLAUDE.md §2 como hard rule. cifra es UN producto visualmente, TRES módulos data-wise.)

> Earlier: 2026-05-04 (stints 65.A-G — brand refresh navy primary + cifra-red demoted to accent + UX fresh-eye batch + remove top-tabs en CRM/tax-ops sidebar.)

> Earlier: 2026-04-30 / 2026-05-01 (stints 64.X.1-14 — post-V audit polish: route status by service_kind, contact kinds, inbox surfaces overdue tasks, redirect /→/tax-ops, matrix toolbar one-line fit, DataTable density alignment, kebab hover-only, empty-state unification, 14 sub-commits.)

> Earlier: 2026-04-27 (stints 50 + 50.D closed — data-integrity emergency en dos rounds. Round 1: contactos backfill (mig 063) + dedup 70→34 con normalización permisiva [,.()]. Round 2: normalización agresiva con TRANSLATE accents + strip todo non-alphanumeric, hard-delete por default, 11 grupos extra mergeados (22→11), 36 soft-deleted previos hard-deleted, 5 merges manuales aprobados, 2 mantenidos como entidades distintas. Mig 065 reemplaza 064 con regex agresiva. Final: 162 active + 0 inactive + 52 merge audits + 0 dupes. 707 tests verde.)
