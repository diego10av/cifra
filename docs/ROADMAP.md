...
| 4 | **Client approval portal (signed link)** | ~~M~~ **DONE (overnight 2)** | HMAC-signed self-contained token (`src/lib/approval-tokens.ts`) → reviewer clicks "Share for approval" in the declaration action bar → `ShareLinkModal` generates link with selectable expiry (1-30 days). Fund manager opens `/portal/[token]` (no login, no sidebar, no auth), sees entity + period + line count + total VAT payable + big Approve button. Click → `POST /api/portal/[token]/approve` validates token, transitions declaration to 'approved', runs precedent upsert (same side-effect as internal approval), writes audit row with IP + user agent + nonce. Replay-safe (tokens are expiring; double-approval returns 409). Commit [shipped]. |
...
| 7 | **Refactor declaration page into subfiles** | ~~M~~ **DONE (partial, overnight)** | 2,637-line monolith → 1,662 lines + 7 extracted modules (_types / _helpers / _atoms / PreviewPanel / OutputsPanel / EmailDrafterModal / FilingPanel). 37% reduction; zero behaviour change. Remaining inline: main orchestration component + ReviewTable/TableRow (they own page state and refactoring risks subtle regressions — deferred). |
...
| 9 | **In-product chat: Haiku default + Ask-Opus button** | ~~M~~ **MVP SHIPPED (overnight)** | Right-side drawer "Ask cifra" in top bar. ✅ Haiku default + Ask-Opus button (re-runs last question on Opus with a cost display). ✅ Context-aware via URL (entity/declaration auto-loaded into system prompt). ✅ Compact legal-sources index in system prompt with pill-rendered citations. ✅ Per-user budget cap with live quota bar in drawer header. ✅ Rate limit 30/min. ✅ Stateless server; client holds conversation. Pending: persist threads in `chat_threads` + `chat_messages` (migration written, not yet surfaced in UI); admin UI at `/settings/users` to raise per-user cap; streaming responses (SSE). |
