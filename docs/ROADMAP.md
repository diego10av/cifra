...
| P1.19 | **RULE 11X — EU supplier charged foreign VAT on a service** | S | Mirror of RULE 17X for goods. Today F105/F106 fall to NO_MATCH — reviewer-facing error message explaining the anomaly would be better. ~20 lines, new fixture, stable flag text. |
| P1.20 | **Legal-watch — direct curia.europa.eu RSS fetcher** | M | Today the scanner uses VATupdate.com as the broad aggregator. A direct curia RSS integration (Recherche Judiciaire publications feed) would be higher-signal, especially for LU-originated references. Complements the stint-18 feed; swap in as an additional source in `legal-watch-scan.ts`. |
