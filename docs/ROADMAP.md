...
>
> Last updated: 2026-04-24 — Stints 37 + 38 closed (13 commits).
> Stint 37 ships full UX polish based on Diego's week-of-usage audit:
> status enum rework (info_to_request + awaiting_client_clarification,
> paid out), sidebar global reorg (Home/VAT/CRM/Tax-Ops/Operations as
> modules), admin tolerance respected in deadline badges, CIT rediseño
> with NWT as inline column, client_groups CRUD with inline family
> assignment, row CRUD (add entity / add ad-hoc / archive) everywhere,
> tasks rediseño for PM semantics (entity + kind + waiting_on +
> follow_up_date), deadline rules CRUD ampliado with adhoc_no_deadline
> kind, entity detail tax-status pills, graceful-degradation test.
> Stint 38: dynamic sidebar from tax_deadline_rules (migration 050) +
> GTM playbook (docs/go-to-market-alt-fund-managers.md). Previously
> stint 36: inline-edit + Excel export; stint 35: tax-type-first
> redesign; stint 34: initial `/tax-ops` module.
...
| ~~11~~ | ~~**Multi-contact per client + auto-inherit to entities**~~ | ~~M~~ | ✅ **Shipped 2026-04-19 in stint 11** (migration `012_client_contacts.sql` — `client_contacts` table + ApproversCard picker pulling from it). |
| ~~12~~ | ~~**Classification: Independent directors (natural + legal persons)**~~ | ~~M~~ | ✅ **Shipped 2026-04-19 in stint 11** — natural-person directors → OUT_SCOPE per C-288/22 TP; legal-person → taxable + CONTESTED flag per AED Circ. 781-2. Citations in `docs/classification-research.md`. |
| ~~13~~ | ~~**Pro-rata computation + UI (mixed-use fund managers)**~~ | ~~L~~ | ✅ **Shipped 2026-04-19 in stint 11** — `src/lib/prorata.ts` + API endpoints + ProrataPanel on `/declarations/[id]` with three-card headline + formula trail + missing-config banner. 11 unit tests. |
| ~~14~~ | ~~**Landing page at cifracompliance.com root**~~ | ~~L~~ | ✅ **Shipped 2026-04-19 in stint 11** at `/marketing` — Factorial + Linear + Veeva + Stripe-inspired. Hero, vertical pitch, how-it-works, 6-stat depth grid, 6 case-law chips, 10-item product arc, mailto CTA. Static-rendered, noindex/nofollow. Brand refresh applied in stint 65.A (navy primary, cifra-red as accent). |
| ~~15~~ | ~~**SPV passive-holding classification hardening**~~ | ~~S~~ | ✅ **Shipped 2026-04-19 in stint 11** — RULE 15P routes SPV domestic leg to LUX_17_NONDED. Citations: Polysar C-60/90, Cibo C-16/00, Marle C-320/17, Larentia+Minerva C-108/14+C-109/14. |
| ~~16~~ | ~~**Carry interest + waterfall distribution rules**~~ | ~~S~~ | ✅ **Shipped 2026-04-19 in stint 11** — keyword-triggered classification rules + flags (carry to pure-service GP → review flag for potential 17% re-characterisation). |
...
| Date | Commit | What |
|------|--------|------|
| 2026-04-25 (morning batch) | bdaba84 → dc7c3e1 (3 commits) | **Cleanup-batch: bulk ops + backup snapshot + orphan-route cleanup.** Diego applied his §11 actionable-first discipline; audit identified one orphan route (/tax-ops/nwt deprecated since stint 37.D, deleted + sidebar map + searchbar entry cleaned). Bulk ops on /tax-ops/entities: per-row checkbox + per-group select-all + "Include archived" toggle + sticky branded toolbar with 3 actions (Change family / Archive / Reactivate, with appropriate modals). New POST /api/tax-ops/entities/bulk-update with strict whitelist + atomic transaction + per-entity audit_log emission. Backup endpoint GET /api/tax-ops/backup returns Content-Disposition-attachment JSON of all 7 tax-ops tables (with optional audit log). Settings card + dedicated /tax-ops/settings/backup page with the download button. 693 tests still green. |
| 2026-04-25 (stint 42) | 943f208 → de8be86 (6 commits) | **Stint 42: overnight feature batch.** Diego gave carte blanche "haz lo que se te ocurra mientras duermo". 6 self-contained commits, all aditivo, all rollback-safe: (F) migration 056 adds `(obligation_id, period_label)` composite index on tax_filings to pre-empt seq-scan degradation at scale; (D) `@media print` CSS rules hide chrome and preserve status-chip colour so matrices print cleanly; (A) entity activity timeline closes the deferred 40.M — new `/api/tax-ops/entities/[id]/timeline` endpoint + `audit-humanize` helper + `EntityTimeline` component turning raw audit_log rows into emoji-tagged one-liners grouped by month; (C) iCal deadline feed — hand-rolled RFC 5545 serializer (no dep), token-guarded `/calendar.ics` endpoint, Settings subscription page with Google/Apple/Outlook instructions, privacy-scoped payload; (B) global contacts book — reverse index over all csp_contacts JSONB arrays + atomic bulk-rename propagation with dry-run preview; (E) dedup "Auto-merge exact matches" bulk button turns confidence-1.00 clusters (punctuation/legal-suffix variants of same entity) into a 1-click batch. 17 new unit tests (audit-humanize + iCal). 693 tests green. Zero destructive migration. |
| 2026-04-24 (stint 41) | 500dd8c | **Stint 41: WHT per-entity cadence switcher.** Migration 055 adds wht_director_quarterly (fifth WHT cadence alongside monthly / semester / annual / adhoc). New POST `/api/tax-ops/obligations/[id]/change-cadence` endpoint that updates tax_type + period_pattern in place, within the wht_director_* family only (prefix safety), rejecting cross-family moves + colliding obligations, full audit log. New cadenceColumn factory + CadenceInlineCell (chip + transparent native `<select>` overlay for 1-click change, with confirm dialog explaining orphan-filings caveat). Wired into WHT monthly / semester / annual matrix pages alongside the already-shipped Family column. Also patched a stale oversight: the Edit-all pencil drawer (from stint 40.G.2) wasn't wired on WHT pages — now consistent. 3 new unit tests covering the cadence option set. 678 tests green. |
| 2026-04-24 (stint 40 top-up) | 0948bca → c83bfc1 (3 commits) | **Stint 40 top-up — deferred items landed same night.** 40.G.2: FilingEditDrawer (pencil ✎ trigger in each matrix row opens a 480px right-side drawer exposing every editable field; Save = 1 PATCH of dirty keys; toast with Undo reverts the diff; wired on 10 matrix pages). 40.P: `/tax-ops/families/[id]` overview with stats + entities table + bulk-copy contacts flow (new GET /api/tax-ops/families/[id] + POST /api/tax-ops/entities/bulk-set-contacts; group header + settings/groups + entity detail all link to the new page). 40.E: wht_director_adhoc deadline rule (migration 054) + Ad-hoc tab in WhtTabs routing to /tax-ops/other for irregular director-fee filings. Full per-entity WHT cadence switcher deferred to stint 41. 675 tests still green. |
| 2026-04-24 (stint 40) | f5df6a0 → 54a565b (13 commits A→N) | **Stint 40: Tax-Ops iteration #3 after second field-usage round.** Entity dedup batch tool in Settings (Levenshtein + Luxembourg legal-form normalisation, 16 new tests, merge endpoint with atomic obligation reassignment). Family hygiene: CTR/FCR fake groups removed (84 entities unassigned). Sidebar restructured: BCL under CIT as single entry (add-entity creates both SBS + 2.16 obligations), FATCA placeholder with custom 404, NWT card removed from Overview grid. Invoice price column (migration 052) on 10 matrix pages with Excel export parity. Contacts column on every matrix row (inline popover reusing CspContactsEditor). CIT assessment chips "Not yet / ✓ Received / ✕ No expected". NWT Review opt-out. Deadline tolerance realigned: CIT 31 Dec → 30 Oct (migration 053). Archive respects current-year (liquidated-in-year still shows; rolls off next year). Back navigation via router.back(). Entity archive/reactivate emit undo toasts. Red stripe softened to gray 2px rail (brand pink reserved for CTAs only). "Gab, Andrew" example text scrubbed (UI + chat prompts). Tasks filter pills relabelled ("My tasks", "Blocked on others", "Due this week"). Tasks Due widget on Tax-Ops overview. QuickCaptureModal defaults assignee to Diego. 675 tests green, 35 test files, build OK. Deferred to stint 41: WHT consolidation, Family overview page, Edit-all drawer. |
| 2026-04-24 (stint 39) | add002b → 7103bc6 (7 commits A→G) | **Stint 39: Tax-Ops iteration #2 after Diego's post-stints-37+38 field usage.** Seven sub-commits addressing a ~10-item punch list. 39.A sidebar reorder (Home → Tax-Ops → VAT → CRM → Operations) + `$` → `€` icons. 39.B family column becomes the first sticky column with djb2-hash colored chips (10-palette), Entity becomes second sticky at `left:170px`, width bumped to `w-[170px]` (Azora no longer truncates). 39.C `yearOptions` helper → dropdown now `[y-2, y-1, y, y+1]` computed at load, and entity detail gets Archive + Reactivate buttons with liquidation_date capture (rollover already filters inactives). 39.D status-filter dropdown on every matrix toolbar (all / 9 statuses / "No status set") → `filterEntitiesByStatus` client-side. 39.E family delete cascade: `?unassign=1` on DELETE /client-groups/[id] nulls out `client_group_id` on referencing entities before dropping the group (Diego's "CTR y CSR no es familia de nada"); applyStatusChange gains optional `toast` param → success toast with Undo that reverses PATCH (prior status) or DELETEs newly-created filing. 39.F migration 051 adds `last_info_request_sent_at DATE` to tax_filings; new `lastChasedColumn` surfaces max-chase-date row-level and writes to every filing on save; matrix API + export both ship the field. 39.G 13 new unit tests (filterEntitiesByStatus, applyStatusChange undo PATCH + DELETE, yearOptions + defaultYear, familyColors determinism), TODO + ROADMAP refreshed. Gate verde por commit: tsc clean, 34 test files, 656 tests, build OK. |
| 2026-04-24 (stints 37+38) | 38eb518 → 5ee455d (13 commits) | **Stints 37 + 38: UX polish from Diego's audit + dynamic sidebar + GTM playbook.** See `docs/TODO.md` "Done this week" for the full per-commit breakdown. Highlights: new status enum (`info_to_request` + `awaiting_client_clarification`, `paid` out as optional fields only); sidebar fully reorganized (Home / VAT / CRM / Tax-Ops / Operations with 2-level nesting, Tax-Ops sidebar dynamically rendered from tax_deadline_rules so archive/create propagates without redeploy); DeadlineWithTolerance shows "within tolerance (Nd left)" amber state; CIT page redesigned with family + Assessment editable + NWT Review collapsed into a column (dedicated NWT page deprecated); client_groups CRUD + inline family reassignment dropdown; row-level CRUD across all 10 matrix pages (+ Add entity, + Add ad-hoc, trash icon archive); tasks extended schema (entity_id, task_kind, waiting_on_kind, waiting_on_note, follow_up_date) + matrix columns + QuickCaptureModal rich form; deadline rules full CRUD including POST/DELETE + adhoc_no_deadline kind (WHT director reclassified); entity detail tax-status summary pills; graceful-degradation test locks 13 core paths AI-free; `/crm/outreach` MVP (prospects tracker + kanban + metrics); `docs/go-to-market-alt-fund-managers.md` 9-section playbook (ICP, channels, templates, cadence, benchmarks, concrete Week 1-4 plan). Migrations 047, 048, 049, 050 applied. Gate green: tsc, 643 tests, build 147+ pages. |
