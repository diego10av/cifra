      ...
      {!preview && (
        <div className="bg-surface border border-border rounded-lg p-5">
          <div className="text-sm text-ink-soft mb-2">
            ...
            <ul className="mt-1 ml-4 list-disc text-xs text-ink-muted">
          ...
          />
          {error && (
            <div className="mt-2 px-3 py-2 bg-danger-50 border border-danger-200 text-xs text-danger-800 rounded">
              {error}
            </div>
          )}
          <div className="mt-3 flex justify-end">
            <button
              onClick={handleParse}
              disabled={!text.trim()}
              className="h-9 px-4 rounded-md bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600 disabled:opacity-50 inline-flex items-center gap-1.5"
            >
      ...
      {preview && !result && (
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-border rounded-lg p-4">
            <h3 className="text-sm font-semibold text-ink mb-2">Column mapping</h3>
            <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
              {header.map((h, i) => (
                <label key={i} className="block">
                  <div className="text-2xs uppercase tracking-wide text-ink-muted mb-0.5">
                    Col {i + 1}: <span className="font-mono">{h}</span>
                  </div>
                  <select
                    value={mapping[i] ?? ''}
                    onChange={e => updateMapping(i, (e.target.value || null) as CanonicalCol | null)}
                    className="w-full border border-border-strong rounded px-2 py-1 text-xs bg-white"
                  >
          ...
          <div className="bg-surface border border-border rounded-lg overflow-hidden">
            ...
            <div className="overflow-x-auto max-h-[420px]">
              <table className="w-full text-xs">
                <thead className="bg-surface-alt text-ink-muted uppercase text-2xs tracking-wide">
                  <tr>
          ...
          {error && (
            <div className="px-3 py-2 bg-danger-50 border border-danger-200 text-xs text-danger-800 rounded">
              {error}
            </div>
          )}
          ...
          <div className="flex justify-between items-center">
            <button
              ...
              className="h-9 px-3 rounded border border-border-strong text-sm text-ink-muted hover:text-ink"
            >
              Change input
            </button>
            <button
      ...
      {result && (
        <div className="bg-surface border border-border rounded-lg p-5">
          ...
          {result.skipped.length > 0 && (
            <div className="mt-5">
              <div className="text-xs uppercase tracking-wide font-semibold text-amber-800 mb-2 inline-flex items-center gap-1.5">
                <AlertTriangleIcon size={12} /> Rows skipped — fix + re-paste
              </div>
              <ul className="bg-amber-50 border border-amber-200 rounded divide-y divide-amber-200 text-xs">
                {result.skipped.map((s, i) => (
                  <li key={i} className="px-3 py-2">
                    <div className="text-amber-900 font-semibold">Row {s.row_index + 1}: {s.reason}</div>
                    <div className="text-amber-800/80 mt-0.5 font-mono text-xs">
          ...
          <div className="mt-5 flex gap-2">
            <button
              ...
              className="h-9 px-4 rounded-md bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600"
            >
              Back to client
            </button>
            <button
