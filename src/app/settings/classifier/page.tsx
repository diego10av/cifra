    ...
    <div>
      ...
      <div className="mb-5 flex items-start justify-between gap-3">
        ...
        <button
          onClick={load}
          disabled={reloading}
          className="h-9 px-3 rounded-md border border-border-strong text-sm font-medium text-ink-soft hover:bg-surface-alt disabled:opacity-50 inline-flex items-center gap-1.5"
        >
      ...
      <div className={[
        ...
        <div className="flex-1 min-w-0">
          <div className="text-2xl font-bold tabular-nums text-ink leading-none" style={{ letterSpacing: '-0.02em' }}>
            {report.passed}/{report.total}
            <span className="text-base text-ink-soft font-semibold ml-2">({pct}%)</span>
          </div>
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden mb-5">
        <div className="px-4 py-2.5 border-b border-border bg-surface-alt/60">
          <h3 className="text-sm font-semibold text-ink">By archetype</h3>
        ...
        <div className="divide-y divide-divider">
          {report.archetypes.map(a => {
            ...
            return (
              <div key={a.archetype} className="px-4 py-2.5 flex items-center gap-3 text-sm">
                ...
                <span className={`text-2xs tabular-nums w-12 text-right font-semibold ${archOk ? 'text-emerald-700' : 'text-warning-700'}`}>
                  {archPct.toFixed(0)}%
                </span>
              </div>
            );
      ...
      {report.failures.length > 0 && (
        <div className="bg-surface border border-danger-200 rounded-xl overflow-hidden mb-5">
          <div className="px-4 py-2.5 border-b border-danger-200 bg-danger-50">
            <h3 className="text-sm font-semibold text-danger-800 flex items-center gap-1.5">
            ...
            {report.failures.map(f => (
              <div key={f.id} className="px-4 py-3 text-sm">
                <div className="flex items-center gap-2 mb-1">
                  <code className="text-2xs bg-surface-alt px-1 py-0.5 rounded font-mono font-semibold">{f.id}</code>
                  <span className="font-medium text-ink">{f.title}</span>
                  {f.legal_ref && (
                    <span className="ml-auto text-2xs text-ink-muted italic">{f.legal_ref}</span>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs mt-2">
                  <div className="bg-emerald-50 border border-emerald-200 rounded px-2 py-1.5">
                    <div className="text-2xs font-semibold text-emerald-800 uppercase mb-0.5">Expected</div>
                  ...
                  <div className="bg-danger-50 border border-danger-200 rounded px-2 py-1.5">
                    <div className="text-2xs font-semibold text-danger-800 uppercase mb-0.5">Got</div>
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        <h3 className="text-sm font-semibold text-ink mb-2">Rules exercised</h3>
        ...
        <div className="flex flex-wrap gap-1.5">
          {report.rules_exercised.map(r => (
            <code key={r} className="text-2xs font-mono bg-surface-alt px-1.5 py-0.5 rounded border border-border">
              {r}
            </code>
          ))}
        </div>
