...
export const FIXTURES: InvoiceFixture[] = [
  ...
  {
    id: 'F105',
    title: 'EU supplier mistakenly charges foreign VAT on services → RULE 11X flag',
    archetype: 'cross_border_advisor',
    ...
    expected: { treatment: null, rule: 'RULE 11X', flag: true },
    notes: 'EU supplier erroneously charged 21% BE VAT on a reverse-charge service. RULE 11X (service mirror of RULE 17X) flags this as an anomaly and directs the reviewer to request a corrected invoice — foreign VAT is not deductible in LU (C-333/20 Wilo Salmson, Art. 49 LTVA).',
  },
  {
    id: 'F106',
    title: 'EU supplier mistakenly charged foreign VAT on fund mgmt → RULE 11X flag',
    archetype: 'fund_admin',
    ...
    expected: { treatment: null, rule: 'RULE 11X', flag: true },
    notes: 'Same pattern as F105: IE supplier erroneously charged 23% IE VAT on a service for a LU fund. RULE 11X catches this symmetrically with F105, pointing the reviewer at Art. 196 Directive — the supplier should issue no VAT, the LU fund self-assesses (or the fund-management exemption applies with no VAT either way).',
  },
  {
    id: 'F111',
    title: 'Non-EU supplier charges origin VAT on a service → RULE 11X flag',
    archetype: 'cross_border_advisor',
    legal_ref: 'Art. 44 / 196 Directive — recipient reverse-charges; supplier abroad issues no LU-side VAT',
    context: HOLDING_CTX,
    input: {
      direction: 'incoming', country: 'CH',
      supplier_name: 'Zurich Consulting GmbH',
      description: 'Strategic advisory — market entry analysis',
      vat_rate: 0.077, vat_applied: 1155, amount_eur: 15000,
    },
    expected: { treatment: null, rule: 'RULE 11X', flag: true },
    notes: 'Covers the non-EU branch of RULE 11X. Swiss supplier charged 7.7% CHF VAT on a service supplied to a LU SOPARFI — should have issued no VAT (Swiss supplier, B2B customer abroad; the LU acquirer self-assesses on RC_NONEU). Swiss VAT is not recoverable in LU (C-333/20; Art. 49 LTVA limits deduction to VAT charged under the Directive). Reviewer requests a corrected invoice or absorbs the VAT as cost.',
  },
  {
    id: 'F112',
    title: 'Ambiguous goods/service description with foreign VAT → RULE 17X wins, not 11X',
    archetype: 'boundary_goods_vs_service',
    legal_ref: 'Rule ordering regression guard — goods path must win when GOODS_KEYWORDS match',
    context: HOLDING_CTX,
    input: {
      direction: 'incoming', country: 'DE',
      supplier_name: 'Hamburg Logistics GmbH',
      description: 'Server deployment and logistics — hardware delivery',
      vat_rate: 0.19, vat_applied: 1900, amount_eur: 10000,
    },
    expected: { treatment: null, rule: 'RULE 17X', flag: true },
    notes: 'Boundary fixture: description contains "server" (GOODS_KEYWORDS) and "deployment" (service-leaning). Classifier must route to RULE 17X (goods path), NOT RULE 11X. Protects against a future refactor reordering the rules such that 11X silently captures goods lines. If this fixture ever flips to RULE 11X, re-check the GOODS_KEYWORDS coverage and the insertion order in classification-rules.ts.',
  },
  {
    id: 'F113',
    title: 'Pre-payment invoice with LU VAT — normal treatment + RULE 30 tax-point decorator',
    archetype: 'prepayment',
    legal_ref: 'Art. 61§1 LTVA (Art. 65 Directive) — chargeability at payment date',
    context: HOLDING_CTX,
    input: {
      direction: 'incoming', country: 'LU',
      supplier_name: 'Études Avocats SARL',
      description: 'Acompte sur honoraires pour dossier M&A — avance de paiement 40%',
      vat_rate: 0.17, vat_applied: 800, amount_eur: 4706,
    },
    expected: { treatment: 'LUX_17', rule: 'RULE 1 + RULE 30', flag: true, flag_includes: 'tax point' },
    notes: 'Primary classification is RULE 1 (LU 17%). The post-processor detects "acompte" / "avance de paiement" and suffixes the rule with "+ RULE 30", forces flag=true, and appends the Art. 61§1 tax-point warning to flag_reason. Treatment unchanged — only the timing audit line is added.',
  },
  {
    id: 'F114',
    title: 'Depositary bank fee — "deposit" substring must NOT trigger RULE 30',
    archetype: 'depositary',
    legal_ref: 'Regression guard — PREPAYMENT_KEYWORDS must not false-match "depositary"',
    context: FUND_CTX,
    input: {
      direction: 'incoming', country: 'LU',
      supplier_name: 'Banque Dépositaire SA',
      description: 'Depositary custody Q2 — securities safekeeping',
      vat_rate: 0.17, vat_applied: 3400, amount_eur: 20000,
    },
    expected: { treatment: 'LUX_17', rule: 'RULE 1', flag: false },
