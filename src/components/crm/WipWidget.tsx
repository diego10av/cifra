  ...
  return (
    <div className="border border-border rounded-lg bg-white overflow-hidden">
      <div className="px-4 py-2.5 flex items-center justify-between border-b border-border">
        ...
        <span className={`text-base font-semibold tabular-nums ${totalAmount > 0 ? 'text-amber-700' : 'text-ink-muted'}`}>
      ...
      {!hasWip ? (
        ...
        <ul className="divide-y divide-border">
          {topMatters.map(m => (
            <li key={m.matter_id}>
              <Link
                ...
                className="flex items-center gap-3 px-4 py-2 hover:bg-surface-alt/50 text-sm"
              >
                <span className="shrink-0 font-mono text-xs text-brand-700 w-[100px] truncate">
                  {m.matter_reference}
                </span>
                <span className="flex-1 min-w-0 truncate text-ink">
                  {m.title}
                  {m.client_name && <span className="text-ink-muted text-xs"> · {m.client_name}</span>}
                </span>
          ...
          {data.matters.length > 5 && (
            <li className="px-4 py-1.5 text-center text-xs text-ink-muted italic border-t border-border">
